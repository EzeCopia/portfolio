...
function showSlides(n) {
    let slides = document.querySelectorAll('.apartado.tercero .presentador .slide');
    ...
    document.querySelector('.apartado.tercero .presentador').style.transform = `translateX(${offset}%)`;

    for (let i = 0; i < totalSlides; i++) {
        slides[i].style.transform = `translateX(${i * 100}%)`;
        if (i === slideIndex) {
            slides[i].querySelector('h3').style.opacity = 1;
            slides[i].querySelector('p').style.opacity = 1;
            slides[i].classList.remove('non-centered');
        } else {
            slides[i].querySelector('h3').style.opacity = 0;
            slides[i].querySelector('p').style.opacity = 0;
            slides[i].classList.add('non-centered');
        }
    }
}
